import { MARKET_MODELS } from "@/lib/market-models/india-dialysis";
...
export function calculateOperationalLeakage(inputs: FacilityInputs) {
  ...
  // Replaced magic number 85 with strict MARKET_MODELS import
  const optimalOccupancy = MARKET_MODELS.OPERATIONS.OPTIMAL_OCCUPANCY_PCT;
  ...
  return {
    ...
    // Replaced hardcoded 0.15 with strict MARKET_MODELS baseline friction target
    optimizedLeakage: totalMonthlyLeakage * MARKET_MODELS.OPERATIONS.BASELINE_FRICTION_PCT,
    netImprovement: totalMonthlyLeakage * (1 - MARKET_MODELS.OPERATIONS.BASELINE_FRICTION_PCT),
  };
}