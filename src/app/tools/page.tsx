...
export default function DPREngineWorkspace() {
  ...
  return (
    <main className="min-h-screen bg-[#010810] pt-24 pb-12 text-white selection:bg-[#D4AF37] selection:text-black flex flex-col h-screen">
      ...
      <header className="h-16 border-b border-white/5 flex items-center justify-between px-6 shrink-0 bg-[#0A1118]">
        ...
        <button className="bg-white hover:bg-gray-200 text-[#010810] px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all flex items-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.1)]">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            <div className="flex bg-[#0A1118] p-1.5 rounded-xl border border-white/5 mb-8">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                  mode === "reuse" ? "bg-[#D4AF37] text-[#010810] shadow-glow-gold" : "text-gray-500 hover:text-white"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                  mode === "single" ? "bg-[#3B82F6] text-white shadow-glow-blue" : "text-gray-500 hover:text-white"
                }`}
              >
        ...
        <section className="flex-1 bg-[#0A1118] p-6 lg:p-8 overflow-y-auto">
          
          {/* Top KPI Row */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
            <MetricCard title="Est. Break-even" value={breakevenMonths > 0 ? `${breakevenMonths.toFixed(1)} Mo` : "∞"} icon={<Clock />} color="blue" />
          ...
          <div className="grid lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="lg:col-span-1">
              <GlassCard accent="gold" className="h-full flex flex-col justify-center items-center text-center p-8">
                <div className="w-16 h-16 rounded-full bg-[#D4AF37]/10 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(212,175,55,0.2)]">
                  <IndianRupee size={28} className="text-[#D4AF37]" />
                </div>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-2">Projected Monthly EBITDA</p>
                <h2 className="text-5xl lg:text-6xl font-black tracking-tighter text-white mb-2">
                  {(monthlyEBITDA / 100000).toFixed(2)} <span className="text-2xl text-gray-500">Lakhs</span>
                </h2>
                <p className={`text-sm font-medium ${monthlyEBITDA > 0 ? "text-green-400" : "text-red-400"}`}>
            ...
            <div className="lg:col-span-2 bg-[#010810] border border-white/5 rounded-3xl p-6 relative overflow-hidden">
              ...
              <div className="h-64 w-full relative flex items-end">
                ...
                <svg className="w-full h-full absolute inset-0 pl-8 pb-6 overflow-visible" preserveAspectRatio="none">
                  ...
                  <line x1="0" y1="50" x2="100%" y2="50" stroke="#3B82F6" strokeWidth="2" strokeDasharray="4 4" className="opacity-50" />
                  
                  {/* Profit Growth Curve (Gold) */}
                  <path 
                    d="M 0 250 L 100 200 L 250 150 L 400 100 L 600 50 L 800 -20" 
                    fill="none" 
                    stroke="#D4AF37" 
                    strokeWidth="4"
                  ...
                  {breakevenMonths > 0 && breakevenMonths <= 24 && (
                    <g transform={`translate(${Math.min(600, breakevenMonths * 25)}, 50)`}>
                      <circle cx="0" cy="0" r="6" fill="#010810" stroke="#D4AF37" strokeWidth="2" className="drop-shadow-[0_0_8px_rgba(212,175,55,1)]" />
                      <text x="-20" y="-15" fill="white" fontSize="10" fontWeight="bold">BEP</text>
                    </g>
                  )}
                </svg>
              </div>
